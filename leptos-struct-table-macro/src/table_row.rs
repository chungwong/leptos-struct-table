...
impl ToTokens for TableRowDeriveInput {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        if let Some(enum_variants) = enum_tokens {
            ...
            let strum_derives = if cfg!(feature = "strum") {
                quote! { , strum::Display, strum::EnumString }
            } else {
                quote! {}
            };
            let strum_attr = if cfg!(feature = "strum") {
                quote! { #[strum(serialize_all = "snake_case")] }
            } else {
                quote! {}
            };
            let extra_derives = quote! { #serde_derives #strum_derives };
            enum_tokens = Some(quote! {
                /// Generated enum, variants are the unskipped fields of the associated struct.
                #[derive(Hash, Clone, Copy, Eq, PartialEq, Ord, PartialOrd, Debug #extra_derives)]
                #strum_attr
